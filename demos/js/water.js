...
class Water{
  constructor(GL){
   ...
   `;
    ...
    const filter = GL.Texture.canUseFloatingPointLinearFiltering() ? gl.LINEAR : gl.NEAREST;
  ...
  stepSimulation(){
    this.textureB.drawTo(()=> {
      this.textureA.bind();
      this.updateShader.uniforms({
        delta: [1 / this.textureA.width, 1 / this.textureA.height]
      }).draw(this.plane);
    });
    this.textureB.swapWith(this.textureA);
  }
